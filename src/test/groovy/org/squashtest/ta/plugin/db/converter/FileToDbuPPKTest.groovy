...
import org.squashtest.ta.plugin.db.converter.FileToDbuPPK;
import org.squashtest.ta.plugin.db.resources.DbUnitPPKFilter;

import spock.lang.Specification

class FileToDbuPPKTest extends Specification {
	...
	def "the properties file is correctly configured"(){
		...
		given :
			...
			FileToDbuPPK converter = new FileToDbuPPK()
		...
		when :
			...
			DbUnitPPKFilter myResource = converter.convert(myFileResource)
	...
	def "the conversion should crash"(){
		...
		given :
			URL url = getClass().getClassLoader().getResource("org/squashtest/ta/plugin/db/converter/badPpkFilter.properties")
			File file = new File(url.toURI())
			FileResource myFileResource = new FileResource(file)
			FileToDbuPPK converter = new FileToDbuPPK()
		when :
